...
use crate::{
  core::crypto::Hash,
  nodev2::{
    message::{Command, Message}, node::Node, peer::{Peer, PeerError}
  },
};
...
pub async fn sync_to_peer(
  ...
  let block_hashes = {
    println!("[SYNC] Fetching block hashes from peer");
    match Peer::request(Arc::clone(&peer), Message::new(Command::GetBlockHashes { start, end: peer_height })).await?.command {
      Command::GetBlockHashesResponse { block_hashes } => block_hashes,
      _ => return Err(PeerError::SyncResponseInvalid)
    }
  };
  println!("[Sync] Fetched block hashes from peer");

  // 2. Find fork point: Take a brief READ LOCK on Node.
  ...
  println!("[SYNC] Replacing chain to longer chain from height: {} (fork index {})", fork_height, fork_index);
  ...
  if new_hashes.is_empty() {
    println!("[SYNC] Already synced!");
    return Ok(());
  }

  // 3. Fetch and validate new blocks (No locks on Node held during I/O or heavy computation)
  let mut new_blocks = Vec::with_capacity(new_hashes.len());
  for bh in new_hashes.iter() {
    // Request block from peer (I/O operation)
    let block = {
      match Peer::request(Arc::clone(&peer), Message::new(Command::GetBlock { block_hash: *bh })).await?.command {
        Command::GetBlockResponse { block } => block.ok_or(PeerError::SyncResponseInvalid)?,
        _ => return Err(PeerError::SyncResponseInvalid)
      }
    };
  ...
  // 4. Apply blockchain changes in a single WRITE LOCK (minimizing lock holding time)
  {
    let mut node_guard = node.write().await; // WRITE LOCK acquired

    let expected_previous_hash = node_guard.blockchain.get_block_hash_by_height(fork_height)
      .ok_or(PeerError::NoForkPoint)?;

    // Anti-race condition check: Verify the fork point is still valid
    if expected_previous_hash != node_guard.blockchain.get_block_hash_by_height(fork_height).ok_or(PeerError::NoBlockHash)? {
      println!("[SYNC] Fork point invalidated by parallel chain update. Aborting sync.");
    ...
    println!("[SYNC] Backed local chain to height: {}, Popped {} blocks", node_guard.blockchain.get_height(), popped_blocks);
