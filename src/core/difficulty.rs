use crate::core::{block::Block, economics::{DIFFICULTY_DECAY_PER_TX, MAX_DIFF_CHANGE, TARGET_TIME, TX_TARGET}, utils::{clamp_f, max_256_bui}};
...
impl DifficultyManager {
  pub fn new(last_timestamp: u64) -> Self {
    DifficultyManager {
      block_difficulty: [u8::MAX; 32],
      tx_difficulty: [u8::MAX; 32],
      last_timestamp,
    }
  }
  ...
  pub fn update_difficulty(&mut self, new_block: &Block) {
    ...
    self.block_difficulty = biguint_to_32_bytes(block_big.min(max_256_bui()).max(BigUint::ZERO));
    ...
    let mut tx_big = BigUint::from_bytes_be(&self.tx_difficulty);
    tx_big = tx_big * BigUint::from(tx_ratio) / BigUint::from(1000u64);
    self.tx_difficulty = biguint_to_32_bytes(tx_big.min(max_256_bui()).max(BigUint::ZERO));
...
pub fn calculate_block_difficulty(block_difficulty: &[u8; 32], tx_count: usize) -> [u8; 32] {
  ...
  biguint_to_32_bytes(big * BigUint::from((DIFFICULTY_DECAY_PER_TX * tx_count as f64 * 1000f64) as u64) / BigUint::from(1000u64))
}
